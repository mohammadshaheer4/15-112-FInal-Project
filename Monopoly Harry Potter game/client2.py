...
class Player:
    def __init__(self,canvas,propertydictionary,position,money,mortgagedProperties,unmortgagedProperties,name,image,turn,socket):
        ...
        self.token.place(x=self.propertydictionary[self.position%39]['X']+(self.name*30),y=self.propertydictionary[self.position%39]['Y'])
    ...
    def afterRoll(self,a,whoseturn,listofusers):
        ...
        sendMessageAll(socket,listofusers[:1],message)
...
class gameplay:
    def __init__(self,geometry,image,title,propertydictionary,socket,listofusers,myturn):
        ...
        self.rollbtn=Button(self.canvas,text='Roll',width=10,height=2,command=self.rolldie)
        self.rollbtn.place(x=881,y=555)
        self.endturnbtn=Button(self.canvas,text='End Turn',width=10,height=2,command=self.endturn)
        ...
        self.player1=Player(self.canvas,self.propertydictionary,0,1500,0,0,1,'./Pictures/redsquare.jpg',self.myturn,self.socket)
        self.player2=Player(self.canvas,self.propertydictionary,0,1500,0,0,0,'./Pictures/redsquare.jpg',0,self.socket)
        flag=True
        while flag:
            self.player1.getMail()
            if self.player1.getMail()!=[]:
                print(self.player1.getMail())
                flag=False
    def rolldie(self):    #function for when i press roll button
...
class connectusers:
    def __init__(self,socket):
        ...
        self.listofusers=['mshaheer','fts']
        self.msgrecv=getMail(self.socket)
        self.msgrecv=self.msgrecv[0][1]
        self.msgrecv=json.loads(self.msgrecv)
        print(self.msgrecv)
        self.myturn=self.msgrecv['fts']


    def startbtnpress(self):
        self.wn1.destroy()
        gameplay('1366x700','./Pictures/gameboard.jpg','Monopoly: Harry Potter Edition',propertydictionary,self.socket,self.listofusers,self.myturn)
