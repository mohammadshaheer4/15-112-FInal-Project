...
class Player:
    def __init__(self,gameplay,position,money,mortgagedProperties,unmortgagedProperties,allProperties,playername,image):
        ...
        self.image=image
        self.turn=self.gameplay.listofusers.index(self.playername)
    ...
    def displayquibblerCard(self,number):
    ...
    def afterRoll(self,a,whoseturn,listofusers):
        ...
        if self.position==2 or self.position==17 or self.position==33:
            ...
            if cardno==2:
                ...
                #self.gameplay.player3.money-=50
                # self.gameplay.l3.config(text=self.gameplay.player3.playername+str(self.gameplay.player3.money))
                #self.gameplay.player4.money-=50
                #self.gameplay.l4.config(text=self.gameplay.player4.playername+str(self.gameplay.player4.money))
            if cardno==3:
                self.displaydailyprophetCard(cardno)
        ...
        self.token.place(x=self.gameplay.propertydictionary[self.position]['X'],y=self.gameplay.propertydictionary[self.position]['Y'])
        
        if self.gameplay.propertydictionary[self.position]['Name'] in self.gameplay.player2.unmortgagedProperties:
        ...
        if self.gameplay.propertydictionary[self.position]['Name'] in self.gameplay.player3.unmortgagedProperties:
            rent=self.gameplay.propertydictionary[self.position].get('Rent',0)
            self.gameplay.player1.money-=self.gameplay.propertydictionary[self.position].get('Rent',0)
            self.gameplay.player3.money+=self.gameplay.propertydictionary[self.position].get('Rent',0)
            self.gameplay.l1.config(text=self.gameplay.player1name+str(self.gameplay.player1.money))
            self.gameplay.l3.config(text=self.gameplay.player3.playername+str(self.gameplay.player3.money))
            tempwn=Tk()
            tempwn.withdraw()
            messagebox.showinfo(self.gameplay.canvas,'This property is owned by '+self.gameplay.player3.playername+' You paid him '+str(rent)+' Galleons as Rent.')
        
        if self.gameplay.propertydictionary[self.position]['Name'] in self.gameplay.player4.unmortgagedProperties:
            rent=self.gameplay.propertydictionary[self.position].get('Rent',0)
            self.gameplay.player1.money-=self.gameplay.propertydictionary[self.position].get('Rent',0)
            self.gameplay.player4.money+=self.gameplay.propertydictionary[self.position].get('Rent',0)
            self.gameplay.l1.config(text=self.gameplay.player1name+str(self.gameplay.player1.money))
            self.gameplay.l4.config(text=self.gameplay.player4.playername+str(self.gameplay.player4.money))
            tempwn=Tk()
            tempwn.withdraw()
            messagebox.showinfo(self.gameplay.canvas,'This property is owned by '+self.gameplay.player4.playername+' You paid him '+str(rent)+' Galleons as Rent.')

        elif self.gameplay.propertydictionary[self.position]['Card'] != None:
...
class gameplay:
    def __init__(self,geometry,image,title,propertydictionary,quibblerCards,dailyProphetCards,socket,listofusers,propertygroups):
        ...
        self.rollbtn=Button(self.canvas,text='Roll',width=10,height=2,state=DISABLED)
        self.rollbtn.bind('<Button-1>',self.rolldie)
        self.rollbtn.place(x=881,y=555)
        self.endturnbtn=Button(self.canvas,text='End Turn',width=10,height=2,state=DISABLED)
        self.endturnbtn.bind('<Button-1>',self.endturn)
        self.endturnbtn.place(x=970,y=555)
        self.buildbtn=Button(self.canvas,text='Build',width=10,height=2,state=DISABLED)
        self.buildbtn.place(x=1059,y=555)
        self.sellbtn=Button(self.canvas,text='Sell',width=10,height=2,state=DISABLED)
        self.sellbtn.place(x=1148,y=555)
        self.mortbtn=Button(self.canvas,text='Mortgage',width=10,height=2,state=DISABLED)
        self.mortbtn.place(x=1237,y=555)
        self.unmortbtn=Button(self.canvas,text='Un-Mortgage',width=10,height=2,state=DISABLED)
        self.unmortbtn.place(x=881,y=613)
        self.trdbtn=Button(self.canvas,text='Trade',width=10,height=2,state=DISABLED)
        self.trdbtn.place(x=970,y=613)
        self.buybtn=Button(self.canvas,text='Buy',width=10,height=2,state=DISABLED)
        self.buybtn.bind('<Button-1>',self.buyProperty)
        self.buybtn.place(x=1059,y=613)
        self.listofusers1=[]
        ...
        self.l3=Label(self.canvas,text=self.player3.playername+str(self.player3.money))
        self.l3.place(x=(965+(0*240)),y=275)
        self.t3=Listbox(self.canvas,height=15,width=35,relief=SUNKEN)
        self.t3.place(x=(881+(0*240)),y=300)
        self.l4=Label(self.canvas,text=self.player4.playername+str(self.player4.money))
        self.l4.place(x=(965+(1*240)),y=275)
        self.t4=Listbox(self.canvas,height=15,width=35,relief=SUNKEN)
        self.t4.place(x=(881+(1*240)),y=300)

    def viewbtnfunc(self):
        index=self.l1.curselection()
        user=self.l1.get(index)
        

    def mortgageProperty(self):
        wn1=Tk()   #the attributes for chat window are set
        wn1.title('Mortgage/Sell Properties')
        wn1.geometry('500x500')
        canvas=Canvas(self.wn1)
        canvas.grid(sticky=N+S+E+W)
        lb1=Label(canvas,text='Current Funds: '+str(self.money))
        lb2=Label(canvas,text='All Properties').grid(row=0,column=0)
        l1=Listbox(canvas,width=20,height=40,relief=SUNKEN)
        l1.grid(row=1,column=0)
        for i in self.unmortgagedProperties:
            l1.insert(END,i['Name']+str(i['Price']))
        mortbtn=Button(canvas,text='Mortgage Property',width=10,height=2,command=self.mortbtnfunc)
        mortbtn.grid(row=3,column=0)
        sellbtn=Button(canvas,text='Sell Property',width=10,height=2,command=self.sellbtnfunc)
        sellbtn.grid(row=3,column=1)
        viewbtn=Button(canvas,text='Done',width=10,height=2,command=self.viewbtnfunc)
        viewbtn.grid(row=3,column=2)
    ...
    def disableall(self):
        ...
        self.trdbtn.config(state=DISABLED)
    ...
    def rolldie(self,event):    #function for when i press roll button
        if (self.whoseturn)==self.player1.turn:
            ...
            self.sellbtn.config(state=NORMAL)
            self.mortbtn.config(state=NORMAL)
            self.unmortbtn.config(state=NORMAL)
            self.trdbtn.config(state=NORMAL)
            self.buybtn.config(state=NORMAL)
    ...
    def buyProperty(self,event):
        if self.propertydictionary[self.player1.position] not in self.player2.allProperties and self.propertydictionary[self.player1.position] not in self.player1.allProperties:
            if self.propertydictionary[self.player1.position]['Price']!=None:
                self.t1.insert(END,self.propertydictionary[self.player1.position]['Name']+'  '+str(self.propertydictionary[self.player1.position]['Price']))
    ...
    def processMsg(self,msglist):
        ...
        if msg['Player']==self.player2.playername:
            ...
            if self.propertydictionary[msg['Position']] in self.player3.unmortgagedProperties:
                self.player3.money+=self.propertydictionary[msg['Position']].get('Rent',0)
                self.l3.config(text=self.player3.playername+str(self.player3.money))

            if self.propertydictionary[msg['Position']] in self.player4.unmortgagedProperties:
                self.player4.money+=self.propertydictionary[msg['Position']].get('Rent',0)
                self.l4.config(text=self.player4.playername+str(self.player4.money))

            self.t2.delete(0,'end')
            for i in range(len(self.player2.allProperties)):    #the list of requests is inserted in the listbox
                self.t2.insert(END,self.player2.allProperties[i]['Name'])

        if msg['Player']==self.player3.playername:
            self.player3.token.place(x=self.propertydictionary[msg['Position']]['X'],y=self.propertydictionary[msg['Position']]['Y'])
            self.player3.jailcards=msg['Jail Cards']
            self.player3.money=msg['Money']
            self.l3.config(text=self.player3.playername+str(self.player3.money))
            self.player3.unmortgagedProperties=msg['Un Mortgaged Properties']
            self.player3.mortgagedProperties=msg['Mortgaged Properties']
            self.player3.allProperties=msg['All Properties']

            if self.propertydictionary[msg['Position']] in self.player1.unmortgagedProperties:
                self.player1.money+=self.propertydictionary[msg['Position']].get('Rent',0)
                self.l1.config(text=self.player1name+str(self.player1.money))

            if self.propertydictionary[msg['Position']] in self.player2.unmortgagedProperties:
                self.player2.money+=self.propertydictionary[msg['Position']].get('Rent',0)
                self.l2.config(text=self.player2.playername+str(self.player2.money))
            ...
            for i in range(len(self.player3.allProperties)):    #the list of requests is inserted in the listbox
                self.t2.insert(END,self.player2.allProperties[i]['Name']+'  '+str(self.player2.allProperties[i]['Price'])+' Galleons')



        if msg['Player']==self.player4.playername:
            self.player4.token.place(x=self.propertydictionary[msg['Position']]['X'],y=self.propertydictionary[msg['Position']]['Y'])
            self.player4.jailcards=msg['Jail Cards']
            self.player4.money=msg['Money']
            self.l4.config(text=self.player4.playername+str(self.player4.money))
            self.player4.unmortgagedProperties=msg['Un Mortgaged Properties']
            self.player4.mortgagedProperties=msg['Mortgaged Properties']
            self.player4.allProperties=msg['All Properties']
            ...
            for i in range(len(self.player4.allProperties)):    #the list of requests is inserted in the listbox
                self.t2.insert(END,self.player2.allProperties[i]['Name']+'  '+str(self.player2.allProperties[i]['Price'])+' Galleons')
...
propertyGroups={'Violet':[1,3],'Sky Blue':[6,8,9],'Purple':[11,13,14],'Orange':[16,18,19],'Red':[21,23,24],'Yellow':[26,27,29],'Green':[31,32,34],'Blue':[37,39]}
wn=Tk()  
